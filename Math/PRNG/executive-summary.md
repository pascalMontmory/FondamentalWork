...
| Multi-seed structural sensitivity | Present through `rho_f`, representative seed selection, and reported dispersion tables. | Raw per-seed tables are not yet imported. |
| `GSQI` and `epsilon(T)` labels | Not yet formalized under these public names. | Add exact definitions or map them explicitly to the existing public formulas before using those names in a public note. |
...
Reported dispersion values show that common PRNGs can exhibit significant seed-dependent variance amplification, especially on rare events. The comparative table is maintained in `verified-results.md` and remains labelled as reported until raw per-seed tables are imported.
