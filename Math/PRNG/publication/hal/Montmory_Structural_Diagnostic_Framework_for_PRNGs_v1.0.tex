...
\hypersetup{
  ...
  pdftitle={Structural Diagnostics for PRNG Stability}
}
...
\title{\textbf{Structural Diagnostics for PRNG Stability}\\
...
\begin{abstract}
Passing a statistical battery such as TestU01 BigCrush is necessary evidence for a pseudo-random number generator, but it does not by itself establish stability of Monte Carlo estimators under changes of seed, integrand, or execution backend. This note proposes a structural diagnostic framework that measures these missing dimensions through seed sensitivity, functional variance dispersion, and cross-backend reproducibility.

The framework introduces finite-window observables, inter-seed dispersion ratios $\rho_f$, a public structural score (GSQI), and a transition-instability measure $\epsilon(T)$. A proprietary 64-bit generator, \texttt{Montmory\_CTACM}, is used only as a controlled test vehicle; its recurrence, constants, and implementation code are not disclosed.

Imported BigCrush logs for seed 42 report 160/160 passed statistics on CPU ARM, Apple Metal GPU, and CUDA GPU backends. Reported structural diagnostics compare \texttt{Montmory\_CTACM} with MT19937, MT19937-64, PCG32, xoshiro256**, Philox4x32-10, and MRG32k3a. The results show that generators compatible with classical batteries can still exhibit large inter-seed dispersion in Monte Carlo variance, especially on rare-event integrands.

The current artifact is computational evidence, not a mathematical proof of generator quality. Its contribution is to make seed-conditioned Monte Carlo stability a reproducible diagnostic target, while identifying the additional artifacts still required for independent verification: raw SmallCrush logs, stream adapter source or hash, backend implementation hashes, bit-exact stream hashes, x86 CPU raw logs, throughput benchmarks, and complete per-seed variance tables.
\end{abstract}
...
The imported BigCrush logs provide computational evidence that \texttt{Montmory\_CTACM} passed BigCrush with 160/160 statistics on CPU ARM, Apple Metal GPU, and CUDA GPU for seed 42. The reported structural dispersion tables indicate seed-dependent variance amplification on practical integrands, especially rare-event indicators, for several tested PRNG families.
