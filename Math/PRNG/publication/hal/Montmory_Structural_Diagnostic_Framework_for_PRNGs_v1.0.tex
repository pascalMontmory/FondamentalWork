...
\hypersetup{
  ...
  pdftitle={A Structural Diagnostic Framework for the Evaluation of PRNG Stability}
}
...
\title{\textbf{A Structural Diagnostic Framework for the Evaluation of PRNG Stability}\\
\large Cross-Architecture Empirical Analysis on ARM CPU, Apple Metal and CUDA}
...
\begin{abstract}
This technical note introduces a structural diagnostic framework for evaluating pseudo-random number generators (PRNGs) in Monte Carlo workloads. The framework is designed to complement, not replace, classical statistical batteries such as TestU01 and PractRand. It focuses on three dimensions that are not directly captured by single-stream distributional tests: seed sensitivity, functional variance dispersion, and cross-backend reproducibility.

A proprietary 64-bit generator, \texttt{Montmory\_CTACM}, is used only as a controlled test vehicle. The generator recurrence, internal constants, and implementation code are not disclosed in this note. The contribution is the public diagnostic methodology and the empirical comparison protocol.

Imported TestU01 BigCrush logs for seed 42 report 160/160 passed statistics on CPU ARM, Apple Metal GPU, and CUDA GPU backends. Structural diagnostics are reported for \texttt{Montmory\_CTACM} and common open or publicly specified generators, including MT19937, MT19937-64, PCG32, xoshiro256**, Philox4x32-10, and MRG32k3a. The results indicate that PRNGs which pass classical batteries can still exhibit significant inter-seed dispersion in Monte Carlo variance, especially on rare-event integrands.

The current public artifact is computational evidence, not a mathematical proof of generator quality. Missing material includes raw SmallCrush logs, stream adapter source or hash, backend implementation hashes, bit-exact stream hashes, x86 CPU raw logs, and complete per-seed variance tables.
\end{abstract}

\noindent\textbf{Keywords:} PRNG, Monte Carlo, TestU01, BigCrush, seed sensitivity, functional variance, reproducibility, GPU, structural diagnostics.
...
Les logs BigCrush importes indiquent 160/160 statistiques passees pour la seed 42 sur CPU ARM, Apple Metal GPU et CUDA GPU. Les diagnostics structurels rapportes montrent que des generateurs passant les batteries classiques peuvent neanmoins presenter une dispersion inter-seed importante de la variance Monte Carlo, notamment sur les evenements rares.
...
\section{Relation to Existing PRNG Evaluation}
...
The second component is empirical distributional testing. TestU01, PractRand, Dieharder, and related tools are designed to identify departures from expected random behaviour over long finite streams. Their role is central. A generator that fails strong statistical batteries requires careful justification before being used in demanding Monte Carlo workloads.

The third component, emphasized here, is workload-facing structural behaviour. In this view, the object of interest is not only the stream considered in isolation, but the effect of seed, integrand, backend, and finite-window replay on quantities used by practitioners. This is why the proposed framework focuses on the empirical variance of Monte Carlo functionals and on reproducibility across backends.

The proposed diagnostics are therefore orthogonal to classical batteries. They do not certify randomness. Instead, they identify whether a generator exhibits unstable behaviour under controlled variations that are common in scientific computing workflows.
...
Structural diagnostics & Is Monte Carlo performance stable under seeds, functionals, and backends? & Conditional empirical variance and replay evidence \\
\bottomrule
\end{tabular}
...
\begin{itemize}[nosep]
  ...
  \item Apple Metal GPU on Darwin;
...
Apple Metal GPU, Darwin & BigCrush & 42 & 160 & all passed \\
...
\begin{tabular}{lllrr}
\toprule
PRNG & Backend & Protocol & $\rho_{u^2}$ & $\rho_{\mathbf{1}_{u>0.99}}$ \\
\midrule
\texttt{Montmory\_CTACM} & CPU & $N=10^6,R=8,100$ seeds & $\sim 10$ & $\sim 3.4$ \\
\texttt{Montmory\_CTACM} & GPU lane 1 & $N=10^6,R=8,100$ seeds & $\sim 10$ & $\sim 3.4$ \\
MT19937 & CPU & $N=10^6,R=8,100$ seeds & $\sim 10$ & $\sim 14$ \\
MT19937-64 & CPU & $N=10^6,R=8,100$ seeds & $\sim 1.9$ & $\sim 25$ \\
PCG32 & CPU & $N=10^6,R=8,100$ seeds & $\sim 15$ & $\sim 3.2$ \\
xoshiro256** & CPU & $N=10^6,R=8,100$ seeds & $\sim 5.9$ & $\sim 10.5$ \\
Philox4x32-10 & CPU & $N=10^6,R=8,100$ seeds & $\sim 4.6$ & $\sim 11$ \\
MRG32k3a & CPU & $N=10^6,R=8,100$ seeds & $\sim 7.7$ & $\sim 3.9$ \\
\bottomrule
\end{tabular}
\caption{Reported inter-seed dispersion ratios on quadratic and rare-event integrands.}
...
The rare-event integrand is particularly informative. Several common generators show larger dispersion on $\mathbf{1}_{\{u>0.99\}}$ than on smoother functionals. This does not imply that these generators are defective in a general sense. It indicates that functional variance can depend on the seed in ways that are not directly summarized by pass/fail battery results.
...
The imported BigCrush logs provide computational evidence that \texttt{Montmory\_CTACM} passed BigCrush with 160/160 statistics on CPU ARM, Apple Metal GPU, and CUDA GPU for seed 42. The reported structural dispersion tables suggest that widely used PRNGs can exhibit significant seed-dependent variance amplification on practical integrands, especially rare-event indicators.

The main conclusion is not that any single generator is universally superior. The main conclusion is that PRNG evaluation should treat seed sensitivity and functional stability as first-class empirical properties, complementary to classical statistical batteries.
...
\newblock Technical documentation and software distribution.
