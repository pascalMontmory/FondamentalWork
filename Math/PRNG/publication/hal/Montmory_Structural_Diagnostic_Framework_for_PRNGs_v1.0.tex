...
Un generateur proprietaire 64-bit est utilise uniquement comme vehicule experimental controle. Sa recurrence interne, ses constantes et son code d'implementation ne sont pas publies dans cette note. La contribution principale porte sur la methode d'evaluation et sur le protocole empirique.
...
\paragraph{Test vehicle.} A proprietary 64-bit PRNG is used as a controlled test vehicle. It is treated as a black-box deterministic PRNG with a fixed public interface:
...
\begin{itemize}[nosep]
  \item a proprietary 64-bit prototype generator, used as a controlled test vehicle;
...
\bottomrule
\end{tabular}
\caption{Imported BigCrush logs for the prototype generator.}
\label{tab:bigcrush}
\end{table}
...
\begin{tabular}{llll}
\toprule
Backend & Backend label & CPU time & Parsed p-value range \\
\midrule
CPU ARM & prototype CPU stream & 02:08:48.69 & 0.02--0.9985 \\
Metal GPU & prototype Metal stream & 02:16:17.77 & 2.6e-3--0.9994 \\
CUDA GPU & prototype CUDA stream & 02:37:03.42 & 2.6e-3--0.9994 \\
\bottomrule
\end{tabular}
...
\midrule
Prototype generator & CPU & $2.0{\times}10^{-7}$ & $2.1{\times}10^{-6}$ & $\sim 10$ & $1.1{\times}10^{-6}$ & $3.8{\times}10^{-6}$ & $\sim 3.4$ \\
Prototype generator & GPU lane 1 & $2.0{\times}10^{-7}$ & $2.1{\times}10^{-6}$ & $\sim 10$ & $1.1{\times}10^{-6}$ & $3.8{\times}10^{-6}$ & $\sim 3.4$ \\
...
\begin{itemize}[nosep]
  \item the prototype generator is proprietary and undisclosed;
...
\paragraph{Layer 3: optional open implementation.} A non-proprietary reference implementation of the diagnostic harness over open generators. This layer does not require disclosure of the proprietary prototype generator. It would make the framework itself reviewable and reusable.
...
The imported BigCrush logs provide computational evidence that the prototype generator passed BigCrush with 160/160 statistics on CPU ARM, Metal GPU, and CUDA GPU for seed 42. The reported structural dispersion tables indicate seed-dependent variance amplification on practical integrands, especially rare-event indicators, for several tested PRNG families.
...
The corresponding repository filenames are listed in the public TestU01 manifest accompanying the raw logs.
