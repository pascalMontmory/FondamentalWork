...
- `PUBLICATION_INDUSTRIELLE.md` : publication courte en Markdown sur les applications industrielles de la cellule de phase.
- `publication_industrial_phase_space_gases.tex` : version LaTeX avec figures.
- `PAPER_QUANTUM_SENSOR_PHASE_SPACE.md` : papier complet sur l'application la plus forte, les capteurs quantiques a atomes froids.
...
- `notes/industrial_phase_space_gases.md` : exploration gaz industriels, moteurs, cryogenie, hydrogene liquide, helium et faisceaux froids.
- `notes/industrial_application_search.md` : recherche d'applications industrielles plus fortes, avec conclusion en faveur des capteurs atomiques et faisceaux haute brillance.
- `scripts/test_velocity_thresholds.py` : test numerique des seuils masse-vitesse avec donnees publiques CODATA/NIST.
- `scripts/simulate_industrial_phase_space_gases.py` : simulations et figures sur le critere `eta = n lambda_th^3`.
...
Conclusion courte: les gaz de chambre d'un moteur chimique sont beaucoup trop classiques pour que `h^3` donne un levier direct de poussee. Le critere devient utile pour cryogenie profonde, hydrogene liquide, helium, faisceaux froids et controle de validite des equations d'etat.

Compiler les publications:
...
Un script local reprend le principe SMTP/MIME de Securaw Attesta sans afficher les secrets:
...
Retirer `--dry-run` pour envoyer effectivement, apres confirmation du destinataire.
