...
7. `COSMO/programme-tests-experimentaux-uvir/`: proposed tests and protocols.
8. `COSMO/cellule-phase-vitesse-gup/`: sensor-oriented phase-space formulation.
9. `COSMO/cellule-phase-deformee-gup/`: deformed phase-cell formalism and supporting derivations.
10. `COSMO/constante-structure-fine-uvir/` and `COSMO/consequences-alpha-uvir-electromagnetisme/`: alpha/electromagnetic conjectures; treat as non-validated unless each file states otherwise.
11. `COSMO/conclusions-cosmologiques-gup-uvir/`: interpretations and open theoretical blocks.
